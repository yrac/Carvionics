...
```
┌──────────────────────────────────────────────┐
│ ECU: OK    SYNC: 0    BAT: 13.8V            │ ← Header
├──────────────────────────────────────────────┤
│                                              │
│  RPM         MAP         CLT                 │ ← Labels (small)
│                                              │
│  2450        42          87                  │ ← Values (large, GREEN)
│                                              │
│  ─────────────────────────────────────       │ ← Row separator
│                                              │
│  IAT         AFR         TPS                 │
...
│                                              │
├──────────────────────────────────────────────┤
│ NORMAL                       FUEL: --.- L   │ ← Footer
└──────────────────────────────────────────────┘

Layout: 3×2 Grid (Six-Pack Avionics)
- Cell Width: 106px each
- Row Height: 100px each
- No borders between cells (clean)
- Values centered, color-coded
```
...
│  RPM         MAP         CLT                 │
│                                              │
│  7850        88          113                 │ ← RED values!
│  (RED)       (AMBER)     (RED)               │
│  ─────────────────────────────────────       │
│                                              │
│  IAT         AFR         TPS                 │
│                                              │
│  38          11.8        65                  │ ← Color changes
│  (AMBER)     (RED)       (AMBER)             │
│                                              │
├──────────────────────────────────────────────┤
│ WARNING                      FUEL: --.- L   │
...
Values berubah warna sesuai threshold:
- RPM > 8000      → RED
- CLT > 110°C     → RED
- AFR < 12.0      → RED
- Battery < 11.0V → Amber di header
```
...
Background: RED ⇄ BLACK (300ms blink)
- SYNC/LOSS: Size 4, centered
- NO ECU: Size 3, centered
- Recovery: Size 2, centered
- Full screen override (no grid)
```
...
## 🖥️ Tampilan per State (Detail)

Berikut detail layout untuk setiap state berdasarkan implementasi:

### 1️⃣ BOOT / POWER ON (Self Test)
...
Duration: ~2 detik
- White box dengan border
- Checklist items (size 2)
- INIT text (size 2, tidak blink)
```
...
Text size 2 (AMBER color)
- Placeholder "---" untuk kejujuran
- Tidak bohong dengan angka fake
```

### 3️⃣ SYNCING (DATA ARRIVING, NOT STABLE)

```
┌──────────────────────────────────────────────┐
│ RECOVERING                                   │ ← Header DARK_GRAY
├──────────────────────────────────────────────┤
│                                              │
│  RPM         MAP         CLT                 │
│  --          --          --                  │
...
Grid 3×2 dengan placeholder "--"
- Label size 1
- Value size 2
- AMBER color untuk placeholders
```
│                                              │
│ WAIT ECU                                     │
│ SERIAL LINK                                  │
...
```
┌──────────────────────────────────────────────┐
│ ECU: OK    SYNC: 0    BAT: 13.9V            │ ← Header (BLACK bg, white line below)
├──────────────────────────────────────────────┤
│                                              │
│  RPM         MAP         CLT                 │ ← Row 1 labels
│  2450        42          87                  │ ← Row 1 values (GREEN)
│                                              │
│  ─────────────────────────────────────       │
│                                              │
│  IAT         AFR         TPS                 │ ← Row 2 labels
│  31          14.4        3                   │ ← Row 2 values (GREEN)
│                                              │
├──────────────────────────────────────────────┤
│ NORMAL                       FUEL: --.- L   │ ← Footer (BLACK bg, white line above)
└──────────────────────────────────────────────┘

Implementation Details:
- Header: ECU status (OK/WAIT), SYNC counter, Battery
- Grid: 3 columns × 2 rows, 106px wide × 100px high each
- No cell borders (clean avionics look)
- Labels: size 1, white, kiri atas
- Values: size 3 (auto-adjust), centered, color-coded
...
│  RPM         MAP         CLT                 │
│  2450        42          97                  │
│  (GREEN)     (GREEN)     (AMBER!)            │ ← Color changes
│  ─────────────────────────────────────       │
│  IAT         AFR         TPS                 │
│  31          16.2        3                   │
│  (GREEN)     (AMBER!)    (GREEN)             │
...
Threshold Triggers:
- CLT > 110°C → AMBER
- AFR > 17.0 or < 12.0 → AMBER
- Values change color, layout tetap sama
```
...
- CLT > 110°C → RED
- AFR < 12.0 → RED
- Battery < 11.0V → Amber warning di header
```
```
