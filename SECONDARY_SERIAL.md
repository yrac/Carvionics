...
6. Secondary IO bersifat independen dari koneksi PC/TunerStudio: setelah di-enable, ECU dapat terus mem-broadcast ke UART yang dipilih meski PC tidak terhubung.
...
- Tidak ada update data:
  ...
  - Pastikan GND ECU terhubung ke GND Arduino (ground bersama).

- Blink SYNC LOSS terus:
  - Pastikan interval kirim CSV stabil (tanpa jeda panjang).
  - Cek threshold dan timeout di `SyncManager`.
  - Jika memakai Binary, beberapa versi status bit dapat berbeda; firmware memiliki fallback yang menetapkan sync setelah beberapa frame valid.
